...
def load_logfile_filename():
    ...
    if 'DNNPORT_MODE_TEST' in os.environ:
        filename = None
    elif 'DNNPORT_LOGFILE_FILENAME' in os.environ:
        filename = os.environ['DNNPORT_LOGFILE_FILENAME']
        throw_error = True
    else:
        filename = os.path.join(os.path.dirname(dnnport.__file__), 'dnnport.log')
    ...
    if filename is not None:
        try:
            ...
            if throw_error:
                ...
                print 'Set the envvar DNNPORT_LOGFILE_FILENAME to fix your configuration.'
                raise
            else:
                filename = None
    return filename
...
def load_logfile_level():
    ...
    elif 'DNNPORT_LOGFILE_LEVEL' in os.environ:
        level = os.environ['DNNPORT_LOGFILE_LEVEL'].strip().lower()
        if level == 'debug':
            return logging.DEBUG
        elif level == 'info':
            return logging.INFO
            ...
            raise ValueError(
                ...
                'Set DNNPORT_LOGFILE_LEVEL to fix your configuration.' % level)
    else:
        return logging.INFO
